...
- Contested items render a `[contested]` marker (3 tokens) in recall, brief, state — and reflex, which is not in FR-2’s list but injects a lone remembered item as settled context, so an unmarked side of an open contest is the worst case of exactly what the marker prevents. It replaces the pre-R1 `[conflict]` marker. `ParsedMemoryItem` carries no conflict column; renderers read the `Conflict: true` line `buildNoteMemoryText` projects into the item text, which is why a note whose own content contains that phrase false-fires — the same flaw the adjacent `status: resolved` sniff has, and both fix with the column, not before.
- **Adjacency is a recall-only guarantee.** `recall` pulls a contested item’s counterparts up to sit directly behind it, pairing on `(scope_key, subject)` because detection is scope-keyed. `brief` sorts by kind priority and `state` renders kind-grouped sections, so a cross-kind contested pair cannot be adjacent there without destroying the primary ordering; the PRD scopes the requirement to recall for this reason. The reorder lives in `recall.ts`, never in `retrieveMemory`: the eval harness reads `top1_hit`/`recall_at_3` straight off retrieval, so display-order changes structurally cannot move ranking metrics.
