...
- One exception to child attribution: an identical `command_failure` inside the 24-hour fold window bumps the existing episode's occurrence count rather than creating a second one, so the episode stays on the session that first recorded it even when a subagent hit the same failure. Splitting it would put two identical items into retrieval, which is what the fold exists to prevent.
