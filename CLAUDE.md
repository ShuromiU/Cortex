...
- **The alternatives line drops before the decision it belongs to, at every budget.** `assembleBudgeted` runs two passes: pass 1 places every affordable primary line exactly as it always did, pass 2 spends only what is left on continuations, top-down, stopping at the first that does not fit. Dropping from the bottom alone would *not* deliver this — with `[d1, alt1, d2, alt2]` a cut landing on `d2` drops that decision and its alternatives together, and `alt1` has already spent budget `d2` needed. When nothing carries alternatives, pass 2 is a no-op and output is byte-identical to before the feature existed.
- The alternatives sniff is **line-prefix** and **note-only**, for the same reasons as the contested sniff plus one that is already live: `eval/suites/budget.json` and `kind-ordering.json` seed decision text whose *content* contains `Alternatives:` mid-sentence, and a substring match pushes `output_tokens` +25 and +18 on those two suites. It reads the **last** matching line, because note content may contain newlines and `buildNoteMemoryText` always writes the real line after the content and `Subject:`.
