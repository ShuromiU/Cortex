...
- The page cap lives in `resolvePageLimit`, not in the CLI action: 20 by default, **200 maximum whatever the caller passes**, with non-finite/zero/negative all falling back to the default. Both guards earn their place, and they fail differently. Commander yields raw strings, so a non-numeric `--limit` arrives as `NaN`, which better-sqlite3 rejects outright — an unguarded crash on a typo. A negative limit is the dangerous one: SQLite reads a negative `LIMIT` as *no limit*, so it would dump the whole store, which is the exact clause AC #1 forbids. (`listMemoryItemsFiltered` passes `-1` deliberately for its own unlimited case; that is why the value must never come from a caller.)
- `inspect-memory` reads `notes.conflict` and `notes.status` **directly** through `source_table='notes'` — the only surface that can, because `ParsedMemoryItem` carries no conflict column and every other renderer recovers the flag from projected text. It reports the column, the projection, and **`diverged` when they disagree**, rather than silently preferring either; that drift is invisible on every other surface. Counterparts are filtered to contested notes **in the same scope**, because detection is scope-keyed while `getActiveNotesBySubject` is not.
