...
- **Serena + RefCertify — this repo is the proof, not the illustration.** `findDbPath` had four copies before Story 2.4, and text search could not see one of them: `src/transports/hook-entry.ts` carried a raw NUL byte, so ripgrep, grep, and `certify_refs`'s text pass all classified it as binary and skipped it **silently** (`rg -n` found two, `rg -na` three). A grep-only enumeration of that symbol returned a confident, complete-looking, wrong answer. `find_referencing_symbols` is the only tool in this repository that would have found it. Treat that as the standing rule here, not an anecdote. **Story 4.5 removed the cause**: all four raw NULs (in `hook-entry.ts`, `reference-validation.ts` and — found by that story's byte-scan, contradicting the recorded set of two — `capture/spool.ts`) were composite-key separators and are now the six-character escape, identical at runtime. A test walks `src/`, `hooks/` and `tests/` and fails on any control byte other than tab, LF or CR, so the enumeration is no longer hand-maintained. The *rule* stands regardless: a symbol query still belongs to the symbol tools, because grep-invisibility was only one of the ways a text enumeration is wrong.
- **Triggers.** Before any rename, signature change, or deletion in `src/db/store.ts`, `src/query/*.ts`, or `src/memory/*.ts`, run `find_referencing_symbols` and act on the returned list. Then `certify_refs` — and read `lspOnly`/`textOnly`, because both being non-zero is the finding. Pass a `symbols` anchor whenever the identifier is a string literal backed by a differently-named symbol: MCP tool names (`cortex_recall`), `source_table` values, hook action arguments, and `REQUIRED_WIRING` command strings are all dispatched by literal and cannot be linked to their handlers by any language server.
- **Loading:** both servers are marked `alwaysLoad`, but Serena takes ~4s to boot and a slow connect still leaves its tools deferred. Load them in one call as your opening action (`ToolSearch "select:mcp__serena__find_symbol,mcp__serena__find_referencing_symbols,mcp__serena__get_symbols_overview,mcp__serena__find_declaration,mcp__serena__find_implementations,mcp__serena__get_diagnostics_for_file,mcp__refcertify__certify_refs"`) rather than concluding they are unavailable. This binds subagents too: when delegating review, refactor, or impact work, name these tools in the prompt — an enumerated command list (build/test/gate/doctor) bounds verification, not exploration.
