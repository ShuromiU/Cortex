...
- **The demotion's durable half lives in the derive layer, not in the transition write.** `deriveMemoryItemState` recomputes every non-pinned/non-archived state from the hotness score on each working-set refresh, and a freshly superseded decision scores hot-range — so a one-time `state='warm'` write would be flipped back to `hot` on the next refresh. Instead the derive rule is: a superseded item lands one tier below what its score would grant. Consequences: refreshes agree with the transition instead of overwriting it, reinforcement (touch raises the score) caps at `warm`, and decay keeps walking it down. `touchMemoryItems` preserves a superseded item's state rather than reheating it, and `stalePenalty` pushes it down like `resolved`. The transition itself demotes **exactly once** — `syncMemoryItemForNote` passes an existing item's state through for superseded notes, because `markConflict`/`clearConflictsForSubject` re-sync every note on a subject and a re-deriving sync would step the tier on every re-sync.
- The superseded sniff (`isSupersededMemoryText`) is **line-exact** and lives in `memory/items.ts`, not `query/render.ts`, because `hotness.ts` needs it and the layer direction forbids `memory/ → query/`. Only the `touchMemoryItems` SQL CASE uses substring LIKE — the same pre-existing divergence the resolved branch carries.
