...
`token_ledger` is a different case and the note that used to sit here was **false**: it is written *and* reported. As of Story 3.5 `cortex stats` prints `Injected / Saved / Net / Efficiency`, plus `Unrealized` and `Estimated` when non-zero and a line explaining a zero `Saved` (`src/transports/cli.ts`, the `stats` command). The credit side is now evidence-only: the consolidation estimate that supplied the entire former `Saved: 657.6k / Efficiency: 93%` is withdrawn, because it credited the difference against pasting every captured event as raw JSON — a baseline no agent would pay, for internal rows that were never injected at all. Verified substitution (Story 4.5) is what will produce real credit; until then `Saved` is honestly 0. Run `cortex stats` before writing anything about this; reading the schema alone is what produced the wrong claim.
