...
`token_ledger` is a different case and the note that used to sit here was **false**: it is written *and* reported. As of Story 3.6 `cortex stats` prints session and scope blocks (`Injected / Saved / Net / Ratio`, plus `Unrealized` and `Estimated` when non-zero) and store-wide retrieval health, built read-only in `src/query/stats.ts` — `Efficiency` is retired, and the ratio is `saved/injected` floored to hundredths. The credit side is evidence-only (Story 3.5): the consolidation estimate that supplied the entire former `Saved: 657.6k / Efficiency: 93%` is withdrawn, because it credited the difference against pasting every captured event as raw JSON — a baseline no agent would pay, for internal rows that were never injected at all. Verified read substitution (Story 4.5) is the mechanism that produces real credit, and it has shipped: when a `Read` returns a file this session already read and the bytes still hash to the recorded digest, the PostToolUse hook replaces the output and emits a `credit` spool line that the flush books as `saved` with evidence. It is **off until `cortex substitution on`**, per project, so `Saved: 0` remains the correct reading for a project that has not enabled it — and `cortex stats` now says which of those two states it is in rather than asserting a global fact about the mechanism. Run `cortex stats` before writing anything about this; reading the schema alone is what produced the wrong claim.
