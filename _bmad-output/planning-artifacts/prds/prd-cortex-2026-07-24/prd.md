...
- **B-4** PostToolUse hook, non-substituting path (`Edit`, `Write`, `Bash`, `Agent`): ≤ 15 ms wall clock, no Node spawn.
- **B-4a** PostToolUse hook, `Read` substitution path: ≤ 100 ms wall clock, no Node spawn. *Amended 2026-07-24 — measured `sha256sum` on a 57 KB payload costs ~54 ms on the target Windows/Git-Bash configuration, so the original 15 ms budget was not achievable for this path. The trade is explicit: ~50–80 ms of hook time to avoid a multi-thousand-token round trip. Architecture must keep the digest lookup Node-free — a flat, greppable digest index alongside the database, not a query — so N-4 still holds. A size ceiling above which substitution is skipped entirely is required.*
